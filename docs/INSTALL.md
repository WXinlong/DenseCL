...
d. Clone the DenseCL repository.
...
2. Following the above instructions, DenseCL is installed on `dev` mode, any local modifications made to the code will take effect without the need to reinstall it (unless you submit some commits and want to update the version number).
...
It is recommended to symlink your dataset root (assuming $YOUR_DATA_ROOT) to `$DENSECL/data`.
...
Here is a full script for setting up DenseCL with conda and link the dataset path. The script does not download ImageNet and Places datasets, you have to prepare them on your own.
